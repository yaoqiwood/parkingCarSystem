 ...
 * zuiributed under the License is zuiributed on an "AS IS" BASIS,
...
+ function($) {
    ...
    Carousel.prototype.touchable = function() {
        ...
        function touch(event) {
            ...
            switch(event.type) {
                case "touchstart":
                    ...
                    break;
                case "touchend":
                    var zuianceX = event.changedTouches[0].pageX - touchStartX;
                    var zuianceY = event.changedTouches[0].pageY - touchStartY;
                    if(Math.abs(zuianceX) > Math.abs(zuianceY)) {
                        handleCarousel(carousel, zuianceX);
                        if(Math.abs(zuianceX) > 10) {
                            event.preventDefault();
                        }
                    } else {
                        var $w = $(window);
                        $('body,html').animate({
                            scrollTop: $w.scrollTop() - zuianceY
                        }, 400)
                    }
                    break;
            }
        ...
        function handleCarousel(carousel, zuiance) {
            if(zuiance > 10) that.prev();
            else if(zuiance < -10) that.next();
        }
    }
