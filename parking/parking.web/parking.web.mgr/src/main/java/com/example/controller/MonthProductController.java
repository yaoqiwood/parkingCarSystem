...
public class MonthProductController {
	 ...
	 * @return json
	 */
	@RequestMapping("/showSellMonthProduct.action") 
	@ResponseBody
	public JSONObject showSellMonthProduct() {
		ArrayList<MonthProduct> proList = monthProductService.showSellMonthProduct();
		JSONObject data = new JSONObject();
		if(proList.size()==0) {
			data.put("type", 0);
			data.put("message", "暂无产品");
		}else {
			data.put("type", 1);
			data.put("list", proList);
		}
