		...
		success : function(msg) {
			...
			if (c == 0) {
				...
				for (var i = 0; i < admins.length; i++) {
					...
					$tr.append($("<td>" +
							"<button adminId='" + admins[i].adminId + "' class='btn btn-sm ' type='button'><i class='icon icon-cog'></i>&nbsp;修改</button>" + 
							"<button adminId='" + admins[i].adminId + "' class='btn btn-sm ' type='button'><i class='icon icon-trash'></i>&nbsp;删除</button>" + 
							"<button adminId='" + admins[i].adminId + "' class='btn btn-sm ' type='button'>" + (admins[i].adminState == "1" ? "<i class='icon icon-remove-circle'></i></i>&nbsp;禁用":"<i class='icon icon-check-circle-o'></i></i>&nbsp;启用") + "</button>"+ 
							"<button adminId='" + admins[i].adminId + "' class='btn btn-sm ' type='button'><i class='icon icon-repeat'></i>&nbsp;重置密码</button>" + 
							"<button adminId='" + admins[i].adminId + "' class='btn btn-sm ' type='button'><i class='icon icon-times'></i>&nbsp;注销</button>"));
							$("#tb").append($tr);
						};
					}
...
$("#updateCommit").click(function() {
	...
	if ($dname.val() == "" || $dstate.val() == "-1" || $drole.val() == "-1" ) {
		zuiAlert("请将信息填写完整！",'danger');
		return;
	}
	var resultNow = postAjax("motifyAdmin.action", {"adminId" : $did.val(), "dname" : $dname.val(), "dstate" : $dstate.val(), "drole" : $drole.val()});
	if(resultNow){
		$("#updatedialog").css("display", "none");
	}
});
...
$("#addcommit").click(function() {
	...
	if ($addname.val() == "" || $addpassword.val() == "" || $addpasswordagain.val() == "" || $addrole.val() == "-1") {
		zuiAlert("不可为空，请输入正确！！",'danger');return;
	}
	if ($addpassword.val() != $addpasswordagain.val()) {
		zuiAlert("两次密码要输入相同！ ",'danger');return;
	}
