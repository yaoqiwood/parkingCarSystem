...
function fillTableBody(param) {
	...
	$("#table").append(
					"<tr class = 'table_content' fid = '"
							...
							+ "</td>"
							+ "<td>"
							+ "<button class='btn btn-sm' id = 'updateBtn' type='button'><i class = 'icon icon-cog'></i>&nbsp;修改</button> "
							+ "<button class='btn btn-sm' type='button'><i class = 'icon icon-remove-circle'></i>&nbsp;"
							+ (state)
							+ "</button>"
							+ "<button class='btn btn-sm' id = 'deleteBtn' type='button'><i class = 'icon icon-times'></i>&nbsp;删除</button>"
							+ "<button class='btn btn-sm' id = 'deleteBtn' type='button'><i class = 'icon icon-list'></i>&nbsp;配置权限</button></td>"
							+ +"</tr >");
};
...
function BtnListener() {
	...
	$("#btnConfirm").click(function() {
		// 增加参数函数
	    if(isChinese($("#addRoleName").val())){
	        addParamFunc();
	    }else{
	        zuiAlert("只能输入中文",'danger');
	    }
	});
